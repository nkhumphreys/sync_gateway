...
TARGET_REPO="https://github.com/nkhumphreys/sync_gateway.git"
...
rewriteManifest () {
    ...
    curl "https://raw.githubusercontent.com/nkhumphreys/sync_gateway/$COMMIT/rewrite-manifest.sh" > rewrite-manifest.sh
    chmod +x rewrite-manifest.sh

    MANIFEST_URL="https://raw.githubusercontent.com/nkhumphreys/sync_gateway/$COMMIT/manifest/default.xml"
...
downloadHelperScripts () {
	...
	curl -s "https://raw.githubusercontent.com/nkhumphreys/sync_gateway/$COMMIT/build.sh" > build.sh
	chmod +x build.sh    
    fi

    if [ ! -f test.sh ]; then
	echo "Downloading test.sh"
	curl -s "https://raw.githubusercontent.com/nkhumphreys/sync_gateway/$COMMIT/test.sh" > test.sh
	chmod +x test.sh
    fi

    if [ ! -f bench.sh ]; then
	echo "Downloading bench.sh"
	curl -s "https://raw.githubusercontent.com/nkhumphreys/sync_gateway/$COMMIT/bench.sh" > bench.sh
	chmod +x bench.sh
    fi

    if [ ! -f snap-manifest.sh ]; then
	echo "Downloading snap-manifest.sh"
	curl -s "https://raw.githubusercontent.com/nkhumphreys/sync_gateway/$COMMIT/snap-manifest.sh" > snap-manifest.sh
...
COMMIT="feature/nathan/custom_sg"
